...
import { fetchSearchShows, fetchShow, fetchShows } from '../fetchShows';
...
test('fetchShows() succes', async () => {
  ...
  await expect(shows).toHaveLength(2);
});
...
test('fetchShow() succes', async () => {
  mockGet(`${Routes.apiShows}/1`).willResolve([
    {
      id: 1,
      url: 'https://www.tvmaze.com/shows/1/under-the-dome',
      name: 'Under the Dome',
...
test('fetchSearchShows() succes', async () => {
  ...
  const shows = await fetchSearchShows('search');
  await flushPromises();
